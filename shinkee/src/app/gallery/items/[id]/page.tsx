...
export default function ItemDetailsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="flex flex-col gap-8 max-w-root w-full mx-auto mb-10 p-8">
      <Link href="/gallery" className="inline-block rounded-xl bg-gray-200 px-3 py-2.5 text-sm opacity-80 transition hover:opacity-100">← Back to list</Link>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {related.map((r) => (
            <Link key={r.id} href={`/items/${r.id}`} className="rounded-2xl bg-gray-200 p-3 transition hover:bg-gray-100 dark:hover:bg-zinc-900/50">
